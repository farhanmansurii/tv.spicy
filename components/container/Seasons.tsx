...
import { ChevronRight, GalleryVerticalEnd, Grid, List, Menu, Play } from 'lucide-react';
...
const SeasonTabs: React.FC<SeasonTabsProps> = ({ seasons, id, tv_id }) => {
	...
	const [activeSeason, setActiveSeason] = useState<number>(
		parseInt(searchParams.get('season') || (seasons && String(seasons[0]?.season)))
	);
					...
					(season: any) =>
						season.season === activeSeason && (
							<div key={season.season} className="flex my-3 h-full flex-col">
								<SeasonContent view={view} id={id} tv_id={tv_id} season={season} />
							</div>
						)
				)}
			</Carousel>
