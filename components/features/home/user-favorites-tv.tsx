...
function UserFavoritesTVComponent() {
	const { data: favorites = [], isLoading } = useUserFavorites('tv');
		...
		const fetchDetails = async () => {
			...
			try {
				const promises = favorites.map(async (fav: any) => {
					try {
						const data = await fetchDetailsTMDB(String(fav.mediaId), 'tv');
...
export const UserFavoritesTV = memo(UserFavoritesTVComponent);
UserFavoritesTV.displayName = 'UserFavoritesTV';
