...
const SECTION_LABEL =
	'text-[9.5px] font-bold uppercase tracking-[0.18em] text-white/30';
...
function buildProviderUrl(provider: WatchProvider, title: string, year: string | undefined, fallback: string) {
...
function ProviderLogo({
	...
	return (
		<a
			...
			className="inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.055] py-1.5 pl-1.5 pr-3 text-[12px] font-semibold text-white/75 transition-all duration-300 hover:border-white/[0.18] hover:bg-white/[0.09] hover:text-white active:scale-[0.97]"
		>
...
function MediaInfoPanelComponent({ data, type, credits, videos = [] }: MediaInfoPanelProps) {
	...
	const creatorLabel = compactList(data?.created_by?.map((creator: any) => creator.name) ?? [], 3);
	const productionLabel = compactList(data?.production_companies?.map((company: any) => company.name) ?? [], 3);
	const networkLabel = compactList(data?.networks?.map((network: any) => network.name) ?? [], 3);
	const countryLabel = compactList(
		data?.origin_country ?? data?.production_countries?.map((country: any) => country.iso_3166_1),
		3
	);
	const keywords = compactList(
		(data?.keywords?.results ?? data?.keywords?.keywords ?? []).map((keyword: any) => keyword.name),
		6
	);
	const voteLabel =
	...
	const providerLink = indiaProviders?.link || `https://www.themoviedb.org/${type}/${data?.id}/watch`;
	...
	const facts = [
		...
		{ label: type === 'tv' ? 'Episode runtime' : 'Runtime', value: runtimeLabel, icon: ClockIcon },
		...
		{ label: type === 'tv' ? 'Created by' : 'Production', value: creatorLabel || productionLabel, icon: UsersThreeIcon },
		{ label: type === 'tv' ? 'Network' : 'Origin', value: networkLabel || countryLabel, icon: GlobeHemisphereWestIcon },
	...
	const externalLinks = [
		...
		{ label: 'Letterboxd', href: letterboxdUrl, detail: type === 'movie' ? 'Search by title' : null },
	...
	const tabs: Array<{ id: PanelTab; label: string; count?: number; disabled?: boolean }> = [
		...
		{ id: 'links', label: 'Links', count: externalLinks.length, disabled: externalLinks.length === 0 },
	];
		...
		<section className="w-full pt-1 md:pt-2">
			<motion.div
				...
				style={{
					...
					boxShadow:
						'inset 0 1px 0 rgba(255,255,255,0.06), 0 24px 64px rgba(0,0,0,0.48)',
				}}
			>
				...
				>
					...
					<div className="relative z-[1] flex min-w-0 flex-1 flex-col gap-1.5">
						<div className="flex items-center gap-2">
							<p className={SECTION_LABEL}>{type === 'tv' ? 'Show info' : 'Movie info'}</p>
							{voteLabel && (
							...
							<div className="flex flex-wrap items-center gap-x-3 gap-y-1">
								{summaryChips.map((chip, idx) => (
									<React.Fragment key={chip.label}>
										{idx > 0 && <span className="text-[10px] text-white/15">|</span>}
									...
									<div className="flex min-w-max gap-1">
										{tabs.map((tab) => (
											<button
												...
												className={`inline-flex h-8 items-center gap-1.5 rounded-full px-3 text-[12px] font-bold transition-all duration-300 active:scale-[0.96] ${
													activeTab === tab.id
											...
											>
												...
												{typeof tab.count === 'number' && tab.count > 0 && (
													<span className={activeTab === tab.id ? 'text-zinc-500' : 'text-white/32'}>
														{tab.count}
													</span>
												)}
											</button>
								...
								>
										...
										<motion.div variants={fadeUp} className="flex flex-col gap-6">
											...
											<div className="flex flex-col gap-3.5">
												...
												{data?.overview ? (
													...
													<p className="text-[13px] text-white/35">No overview available.</p>
												)}
											</div>
									...
									{activeTab === 'watch' && (
										<motion.div variants={fadeUp} className="flex flex-col gap-5">
											<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
												<div>
													<SectionHeading title="Watch in India" kicker={providerTotal ? `${providerTotal} providers` : undefined} />
											...
											{providerGroups.length > 0 ? (
												<div className="space-y-4 border-t border-white/[0.06] pt-4">
													{providerGroups.map((group) => (
														<div key={group.label}>
															<p className={`${SECTION_LABEL} mb-2.5`}>{group.label}</p>
									...
									{activeTab === 'cast' && (
										<motion.div variants={fadeUp} className="flex flex-col gap-4">
											<SectionHeading title="Cast & crew" kicker={`${people.length} people`} />
												...
												<ul className="flex snap-x gap-3 overflow-x-auto pb-1 pr-1 scrollbar-none">
													{people.map((person, i) => (
														...
														>
															...
															<p className="mt-0.5 truncate text-[10.5px] text-white/40">{person.role}</p>
														</motion.li>
													))}
												</ul>
											</div>
									...
									{activeTab === 'trailers' && (
										<motion.div variants={fadeUp} className="flex flex-col gap-4">
											<SectionHeading title="Trailers & previews" kicker={`${videos.length} videos`} />
											<div className="overflow-hidden border-t border-white/[0.06] pt-4">
												<ul className="flex snap-x gap-3 overflow-x-auto pb-1 pr-1 scrollbar-none">
													{videos.map((video, i) => (
														<motion.li
														...
														>
															<button
																...
																className="group relative aspect-video w-full overflow-hidden rounded-xl bg-white/[0.045] text-left transition-transform duration-300 active:scale-[0.98]"
															>
																<Image
									...
									{activeTab === 'links' && (
										<motion.div variants={fadeUp} className="flex flex-col gap-4">
											<SectionHeading title="External pages" kicker={`${externalLinks.length} sources`} />
												...
												{externalLinks.map((link, i) => (
													...
													>
														<span className="flex flex-col">
															...
															{link.detail && (
																<span className="mt-0.5 text-[11px] text-white/35">{link.detail}</span>
															)}
														</span>
														<LinkIcon
															size={16}
			...
			{activeVideo && (
				...
				>
					<div className="relative aspect-video w-full max-w-5xl" onClick={(event) => event.stopPropagation()}>
						<button
							type="button"
							onClick={() => setActiveVideo(null)}
