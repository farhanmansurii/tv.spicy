    ...
    <div className="w-full space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <Dialog key={video.id}>
            <DialogTrigger asChild>
              <button
                ...
                className="group relative aspect-video w-full overflow-hidden rounded-card md:rounded-card-md bg-black/40 shadow-lg ring-1 ring-white/10 transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] hover:ring-white/20"
              >
                {/* Thumbnail */}
                <img
      ...
      {!showAll && sortedVideos.length > 6 && (
        <button
          ...
          className="w-full py-4 text-center text-sm text-white/50 font-medium hover:text-white transition-colors"
        >
