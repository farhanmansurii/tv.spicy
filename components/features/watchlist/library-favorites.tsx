...
import { fetchBasicDetailsFromApi } from '@/lib/api/tmdb-row-client';
			...
			try {
				const promises = likedIds.map(async (id) => {
					try {
						...
						const data = await fetchBasicDetailsFromApi(String(id), 'tv').catch(() =>
							fetchBasicDetailsFromApi(String(id), 'movie')
						);
						return data;
					} catch (error) {
	...
	return (
		<div className="space-y-10">
			...
			<div className="flex items-center justify-between">
				<div>
					<h2 className="text-2xl md:text-3xl font-bold text-foreground">My Favorites</h2>
